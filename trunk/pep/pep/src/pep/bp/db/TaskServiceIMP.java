     ...
     public List<TermTaskDAO> getPollingTask(int CircleUnit){
         try {
            ...
            for (TermTaskDAO task : results) {
                ...
                sbSQL.append(" from G_TASK_DATAITEM");
                sbSQL.append(" where protocol_no = ?");
                sbSQL.append(" and SYS_OBJECT = ?");
                sbSQL.append(" and task_id = ?");
                List<CommanddItemDAO> commandItemList = (List<CommanddItemDAO>) jdbcTemplate.query(SQL, new Object[]{task.getProtocol_No(),task.getSystem_Object(),task.getTaskId()},new CommandItemRowMapper());             
