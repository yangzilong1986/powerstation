...
function Folder(folderDescription, hreference) { //constructor
    ...
    this.linkHTMLEx = linkFolderHTMLEx;
 ...
 *
 */
function drawFolderEx(folder, insertAtObj) {
    var nodeName = "";
    var auxEv = "";
    var docW = "";
    var i = 0;

    finalizeCreationOfChildDocs(folder);

    var leftSide = leftSideHTML(folder.leftSideCoded);

    if(browserVersion > 0) {
        auxEv = "<a href='javascript:clickOnNode(\"" + folder.getID() + "\")'>";
    }
    else {
        auxEv = "<a>";
    }

    nodeName = folder.nodeImageSrc();

    if(folder.level > 0) {
        if(folder.isLastNode) { //the last child in the children array
            leftSide = leftSide + "<td valign=top>" + auxEv + "<img name='nodeIcon" + folder.id + "' id='nodeIcon" + folder.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>";
        }
        else {
            leftSide = leftSide + "<td valign=top background=" + ICONPATH + "ftv2vertline.gif>" + auxEv + "<img name='nodeIcon" + folder.id + "' id='nodeIcon" + folder.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>";
        }
    }
    ...
    docW = folder.blockStartHTML("folder");

    docW = docW + "<tr>" + leftSide + "<td valign=top>";
    if(USEICONS) {
        docW = docW + folder.linkHTMLEx(false);
        docW = docW + "<img id='folderIcon" + folder.id + "' name='folderIcon" + folder.id + "' src='" + folder.iconImageSrc() + "' border=0></a>";
    }
    else {
        if(folder.prependHTML == "") {
            docW = docW + "<img src=" + ICONPATH + "ftv2blank.gif height=2 width=2>";
        }
    }

    if(WRAPTEXT) {
        docW = docW + "</td>" + folder.prependHTML + "<td valign=middle width=100%>";
    }
    else {
        docW = docW + "</td>" + folder.prependHTML + "<td valign=middle nowrap width=100%>";
    }

    if(folder.link != null) {
        docW = docW + folder.linkHTMLEx(true);
        docW = docW + folder.desc + "</a>";
    }
    else {
        docW = docW + folder.desc;
    }
    docW = docW + "</td>";

    docW = docW + folder.trailHTML;

    docW = docW + folder.blockEndHTML();
    ...
    if(browserVersion == 2)  {
        folder.navObj = doc.layers["folder" + folder.id];
        if(USEICONS) {
            folder.iconImg = folder.navObj.document.images["folderIcon" + folder.id];
        }
        folder.nodeImg = folder.navObj.document.images["nodeIcon" + folder.id];
        doc.yPos = doc.yPos + folder.navObj.clip.height;
    }
    else if(browserVersion != 0) {
        folder.navObj = getElById("folder" + folder.id);
        if(USEICONS) {
            folder.iconImg = getElById("folderIcon" + folder.id);
        }
        folder.nodeImg = getElById("nodeIcon" + folder.id);
    }
}
 ...
 *
 */
function linkFolderHTMLEx(isTextLink) {
    var docW = "";

    if(this.link != null) {
        docW = docW + "<a href='" + this.link + "' target=\"" + this.target + "\" ";
...
function drawItem(insertAtObj) {
    ...
    if(USEICONS) {
        if(fullLink != null) {
            ...
            docW = docW + "<img id='itemIcon" + this.id + "' " + "src='" + this.iconSrc + "' border=0>";
        }
    }
    else {
        if(this.prependHTML == "") {
 ...
 *
 */
function drawItemEx(item, insertAtObj) {
    var leftSide = leftSideHTML(item.leftSideCoded);
    var docW = "";

    var fullLink = null;
    if(item.link != null && item.target != null) {
        fullLink = "href=\"" + item.link + "\" target=\"" + item.target + "\" onClick=\"clickOnLink('" + item.getID() + "\', '" + item.link + "','" + item.target + "');return false;\"";
	}
	else {
	    fullLink = null;
	}
    ...
    if(item.level > 0) {
        if(item.isLastNode) { //the last 'brother' in the children array
            leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2lastnode.gif' width=16 height=22></td>";
        }
        else {
            leftSide = leftSide + "<td valign=top background=" + ICONPATH + "ftv2vertline.gif><img src='" + ICONPATH + "ftv2node.gif' width=16 height=22></td>";
    ...
    docW = docW + item.blockStartHTML("item");

    docW = docW + "<tr>" + leftSide + "<td valign=top>";
    if(item.imagesrc != null) {
        if(fullLink != null) {
            docW = docW + "<a " + fullLink  + " id=\"itemIconLink" + item.id + "\">" + "<img id='itemIcon" + item.id + "' " + "src='" + ICONPATH + item.imagesrc + "' border=0>" + "</a>";
        }
        else {
            docW = docW + "<img id='itemIcon" + item.id + "' " + "src='" + ICONPATH + item.imagesrc + "' border=0>";
        }
    }
    else {
        if(item.prependHTML == "") {
    ...
    if(WRAPTEXT) {
        docW = docW + "</td>" + item.prependHTML + "<td valign=middle width=100%>";
    }
    else {
        docW = docW + "</td>" + item.prependHTML + "<td valign=middle nowrap width=100%>";
    }

    if(fullLink != null) {
        docW = docW + "<a " + fullLink + " id=\"itemTextLink" + item.id + "\">" + item.desc + "</a>";
    }
    else {
        docW = docW + item.desc;
    }
    ...
    docW = docW + item.trailHTML;

    docW = docW + item.blockEndHTML();
    ...
    if(browserVersion == 2) {
        item.navObj = doc.layers["item" + item.id];
        if(USEICONS) {
            item.iconImg = item.navObj.document.images["itemIcon" + item.id];
        }
        doc.yPos = doc.yPos + item.navObj.clip.height;
    }
    else if(browserVersion != 0) {
        item.navObj = getElById("item" + item.id);
        if(USEICONS) {
            item.iconImg = getElById("itemIcon" + item.id);
        }
    }
}
...
function leftSideHTML(leftSideCoded) {
    ...
    for(i = 0; i < leftSideCoded.length; i++) {
        ...
        else if(leftSideCoded.charAt(i) == "0") {
 ...
 *
 */
function setFolderEx(type, description, linkData) {
    if(!(type == 1)) {
        //Target changed from numeric to string in Aug 2002, and support for numeric style was entirely dropped in Mar 2004
        alert("Change your Treeview configuration file to use the new style of target argument in setFolderEx");
        return;
    }
    newFolder = new Folder(description, null);
    setFolderLinkEx(newFolder, type, linkData);
    return newFolder;
}
 ...
 *
 */
function setFolderLinkEx(folder, type, linkData) {
    var target = "";
    var link = "";
    ...
    if(type == 1) {
        ...
        if(linkData != null) {
            folder.link = "javascript: selectItem(" + linkData + ");";
        }
        else {
            folder.link = null;
        }
...
function gLnk(optionFlags, description, linkData) {
    if(optionFlags != null && optionFlags >= 0) { //is numeric (old style) or empty (error)
        //Target changed from numeric to string in Aug 2002, and support for numeric style was entirely dropped in Mar 2004
...
function setItemLink(item, optionFlags, linkData) {
    ...
    if(optionFlags == null || linkData == null) {
        item.link = null;
        item.target = null;
        return;
    }
    ...
    else if(targetFlag == "P") {
        target = "_parent";
    }
    else if(targetFlag == "R") {
        target = "basefrm";
    }
    else if(targetFlag == "S") {
        target = "_self";
    }
 ...
 *
 */
function setItemEx(type, description, linkData, image) {
    if(!(type == 1)) {
        //Target changed from numeric to string in Aug 2002, and support for numeric style was entirely dropped in Mar 2004
        alert("Change your Treeview configuration file to use the new style of target argument in setItemEx");
        return;
    }
    newItem = new Item(description);
    setItemLinkEx(newItem, type, linkData, image);
    return newItem;
}
 ...
 *
 */
function setItemLinkEx(item, type, linkData, image) {
    var target = "";
    var link = "";
    var imagesrc = "";
    ...
    if(type == 1) {
        ...
        if(linkData != null) {
            item.link = "javascript: selectItem(" + linkData + ");";
        }
        else {
            item.link = null;
        }
