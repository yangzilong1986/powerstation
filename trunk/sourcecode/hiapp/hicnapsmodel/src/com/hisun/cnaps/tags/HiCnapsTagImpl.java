...
public class HiCnapsTagImpl implements HiCnapsTag {
    ...
    public HiCnapsTagImpl(Element element) {
        ...
        if (!(StringUtils.isBlank(_dataType))) this.dataType = (_dataType.charAt(0) & 0xFF);
        else this.dataType = 110;
        String _length = element.attributeValue("length");

        if (!(StringUtils.isBlank(_length))) try {
    ...
    public void parse(String buffer) throws HiException {
        this.value = buffer;
    }
    ...
    public String parse(HiETF etf) throws HiException {
        if (StringUtils.isNotBlank(this.repeatName))
