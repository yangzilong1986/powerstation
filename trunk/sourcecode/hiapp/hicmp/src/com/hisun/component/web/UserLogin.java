...
public class UserLogin {
    ...
    private boolean appendItem(StringBuffer buf, Element element) {
        ...
        if (element.getName().equals("Group")) {
            ...
            if (append) {
                ...
                if (!(this.gids.contains(s))) this.gids.add(s);
            }
        } else if (element.getName().equals("Item")) {
            String id = element.attributeValue("id");
            if ((getPagePermission(id)) && (this.pages.containsKey(id))) {
                append = true;
            }
    ...
    public String getVmenu() {
        ...
        for (int i = 0; i < this.gids.size(); ++i) {
            if (i == 0) ret.append(showVMenu((String) this.gids.get(i), true));
            else ret.append(showVMenu((String) this.gids.get(i), false));
            ret.append("\r\n");
        }
        ...
        for (Iterator i = vmenu.elementIterator(); i.hasNext();) {
            ...
            if (element.getName().equals("Item")) {
                ...
                if ((getPagePermission(id)) && (this.pages.containsKey(id))) {
                    ...
                    if ((!(StringUtils.isEmpty(url))) && (!("#".equals(url)))) buf.append(url);
                    else {
