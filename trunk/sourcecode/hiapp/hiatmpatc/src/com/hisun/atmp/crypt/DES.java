...
public class DES {
    ...
    private byte[] Encrypt(int[] timeData, int flag, int[][] keyarray) {
        ...
        if (flags == 1) for (i = 0; ; ++i) {
            if (i >= 16) break label117;
    ...
    private void GetEncryptResultOfByteArray(int[] data, byte[] value) {
        for (int i = 0; i < 8; ++i) {
            for (int j = 0; j < 8; ++j) {
                int tmp20_19 = i;
                byte[] tmp20_18 = value;
                tmp20_18[tmp20_19] = (byte) (tmp20_18[tmp20_19] + (data[((i << 3) + j)] << 7 - j));
            }
        }
        for (i = 0; i < 8; ++i) {
            int tmp61_60 = i;
            byte[] tmp61_59 = value;
            tmp61_59[tmp61_60] = (byte) (tmp61_59[tmp61_60] % 256);
            if (value[i] > 128) {
                int tmp80_79 = i;
                byte[] tmp80_78 = value;
                tmp80_78[tmp80_79] = (byte) (tmp80_78[tmp80_79] - 255);
            }
        }
    }
