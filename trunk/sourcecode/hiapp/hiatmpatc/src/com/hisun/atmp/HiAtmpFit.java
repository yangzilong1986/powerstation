...
public class HiAtmpFit {
    ...
    public int GetCrdFlg(HiATLParam argsMap, HiMessageContext ctx) throws HiException {
        ...
        for (; i < fitTable.size(); ++i) {
            ...
            if (isHandInputCard) {
                ...
                if (!(StringUtils.equals(fitValue, fit2))) continue;
            } else {
                int idx;
                String trk = null;

                if (item.fitTrk == 2) trk = trk2;
                else {
                    trk = trk3;
                }
    ...
    public int DESEncrypt(HiATLParam argsMap, HiMessageContext ctx) throws HiException {
        HiETF root = ctx.getCurrentMsg().getETFBody();
    ...
    public int XOREncrypt(HiATLParam argsMap, HiMessageContext ctx) throws HiException {
        HiETF root = ctx.getCurrentMsg().getETFBody();
        String node = argsMap.get("node");
    ...
    public static int ParseFile(HiATLParam argsMap, HiMessageContext ctx) throws HiException {
        HiETF root = ctx.getCurrentMsg().getETFBody();
        Logger log = HiLog.getLogger(ctx.getCurrentMsg());
    ...
    public static int Random(HiATLParam argsMap, HiMessageContext ctx) throws HiException {
        HiETF root = ctx.getCurrentMsg().getETFBody();
        Logger log = HiLog.getLogger(ctx.getCurrentMsg());
        ...
        int ret = r.nextInt(n);
        if (ret == 1) root.setChildValue("result", "1");
        else root.setChildValue("result", "0");
        return 0;
    }
    ...
    public int ReloadCrdFlg(HiATLParam argsMap, HiMessageContext ctx) throws HiException {
        HiFit.reLoadFit(ctx);
        return 0;
    }
