...
import org.pssframework.persist.functional.security.RoleManagerTest.RoleColumn;
import org.pssframework.persist.functional.security.UserManagerTest.UserColumn;
...
public class SecurityCheckingTest extends BaseSeleniumTestCase {
	...
	public void checkAnonymous() {
		//访问退出登录页面,退出之前的登录
		selenium.open("/org.pssframework.persist/j_spring_security_logout");
		assertTrue(selenium.isElementPresent(LOGIN_BUTTON));

		//访问任意页面会跳转到登录界面
		selenium.open("/org.pssframework.persist/security/user.action");
		assertTrue(selenium.isElementPresent(LOGIN_BUTTON));
	}
