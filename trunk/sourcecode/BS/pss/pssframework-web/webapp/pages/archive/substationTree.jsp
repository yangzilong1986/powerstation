      ...
      <ul>
        ...
        <li id="ml"><div><img src="<peis:contextPath/>/img/tree/object_line.gif" alt="" border="0" width="15" height="15" align="bottom"> 母线</div>
            <ul>
              ...
              <li class="closed" onclick="queryMeterMp('MPQ','${MSG.SUBS_ID}','222')" id="222">
                		<table style="display:inline;margin-left:-2pt;margin-top:-3pt;" ><tr>
              			  <td><img src="<peis:contextPath/>/img/tree/object_line.gif" alt="" width="15" height="15" align="bottom"></td>
              		    <td> 其它</td>
              		  </tr></table>
        ...
        <li id="tranz"><div><img src="<peis:contextPath/>/img/tree/object_tg.gif" alt="" border="0" width="15" height="15" align="bottom"> 站用变</div>
        	<ul>
              <c:forEach items="${ETF.TRANZ}" var="MSG">
                <li class="closed" onclick="queryMeterMp('TRANZ','${MSG.OBJECT_ID}','${MSG.TRAN_ID}')" name="queryMeterMp" id="${MSG.TRAN_ID}">
                		<table style="display:inline;margin-left:-2pt;margin-top:-3pt;" ><tr>
              			  <td><img src="<peis:contextPath/>/img/tree/object_tg.gif" alt="" width="15" height="15" align="bottom"></td>
              		    <td onclick="select(this,'${MSG.TRAN_ID}','${MSG.OBJECT_ID}','TRAN')"> ${MSG.TRAN_NAME}</td>
              		  </tr></table>
        ...
        <li id="else"><div><img src="<peis:contextPath/>/img/tree/object_tg.gif" alt="" border="0" width="15" height="15" align="bottom"> 其它设备</div>
           <ul>
              <c:forEach items="${ETF.MP}" var="MSG">
              	<li class="closed" id="${MSG.MP_ID}">
              		<table style="display:inline;margin-left:-2pt;margin-top:-3pt;" ><tr>
              			<td><img src="<peis:contextPath/>/img/tree/object_org.gif" alt="" width="15" height="15" align="bottom"></td>
              		  <td onclick="select(this,'${MSG.MP_ID}','${MSG.SUBS_ID}','MP')"> ${MSG.MP_NAME}</td>
              		</tr></table>
              	</li>
              </c:forEach>
            </ul> 
...
function queryMeterMp(FLG,SUBS_ID,liid){
    ...
    if(objectI=="SUBS"){
      window.parent.mainframe.location.href="112034.dow?SUBS_ID="+tempSUBS_ID+"&FLG=QUERY&OPEN_NEW=substation.jsp";
    }
    if(objectI=="TERM"){
      window.parent.mainframe.location.href="112035.dow?TERM_ID="+objectId+"&SUBS_ID="+subsId+"&FLG=QUERY";
    }
    if(objectI=="TRAN"){
      window.parent.mainframe.location.href="112030.dow?TRAN_ID="+objectId+"&SUBS_ID="+subsId+"&FLG=QUERY";
    }
    if(objectI=="ML"){
  	  window.parent.mainframe.location.href="112031.dow?ML_ID="+objectId+"&SUBS_ID="+subsId+"&FLG=QUERY";
    }
    if(objectI=="MP"){
