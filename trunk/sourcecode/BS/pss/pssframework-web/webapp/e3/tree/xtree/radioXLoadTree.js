...
function WebFXRadioLoadTree(sText, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon, sOpen) {
	// call super
...
function WebFXLoadRadioTreeItem(sText, sValue, sAction, sXmlSrc, eParent, sIcon, sOpenIcon, bChecked, disabled) {
	// call super	
...
function radio_createNode(oNode, parentDisabled) {
	...
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadRadioTreeItem(text, value, action, src, parent, icon, openIcon, checked, disabled);
	else
		jsNode = new WebFXRadioTreeItem(text,value, action, parent, icon, openIcon, checked, disabled);
   
   if(checked){
	  _setCheckedObject(jsNode.id);
   }
