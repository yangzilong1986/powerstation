...
public class JdbcPageQueryUtils {
	...
	public static List pageQuery(NamedParameterJdbcTemplate jdbcTemplate, Dialect dialect,
			SqlOffsetLimit sqlOffsetLimit, final Map paramMap, final RowMapper rowMapper) {
		sqlOffsetLimit = toLimitSql(dialect, sqlOffsetLimit, paramMap);
		return (List) jdbcTemplate.query(sqlOffsetLimit.sql, paramMap, new OffsetLimitResultSetExtractor(
				sqlOffsetLimit.offset, sqlOffsetLimit.limit, rowMapper));
	}
	...
	public static List pageQuery(JdbcTemplate jdbcTemplate, Dialect dialect, SqlOffsetLimit sqlOffsetLimit,
			PreparedStatementSetter pss, final RowMapper rowMapper) {
		sqlOffsetLimit = toLimitSql(dialect, sqlOffsetLimit);
		return (List) jdbcTemplate.query(sqlOffsetLimit.sql, pss, new OffsetLimitResultSetExtractor(
				sqlOffsetLimit.offset, sqlOffsetLimit.limit, rowMapper));
	}
