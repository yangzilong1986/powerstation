 ...
 public abstract class HiAbstractChannelService
 {
   ...
   protected void bindsRqData(HiMessage msg, HttpServletRequest request, HttpServletResponse response)
   {
     ...
     while (en.hasMoreElements()) {
       ...
       if (value instanceof HashMap) {
         ...
         while (iter.hasNext()) {
           ...
           if (!(valueMap.get(name1) instanceof String)) {
             continue;
           }
     ...
     if (request.getQueryString() == null)
       root.setChildValue("REQ_URL", request.getRequestURL().toString());
     else {
   ...
   public void init(ServletConfig config) throws ServletException {
     ...
     String tmOut = config.getInitParameter("tmOut");
     if (StringUtils.isNotBlank(tmOut)) {
       try {
         BeanUtils.setProperty(this._serviceBean, "tmOut", tmOut);
       }
       catch (Exception e1)
       {
       }
     ...
     if (StringUtils.isNotBlank(file)) {
       ...
       if (StringUtils.isBlank(this._redirectURL))
         throw new ServletException("redirectURL not config");
       try
       {
