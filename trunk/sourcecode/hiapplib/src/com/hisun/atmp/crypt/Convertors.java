 ...
 public class Convertors
 {
   ...
   {
     return new Convertor(map, m) { private final Convertors.Map2 val$map;
   ...
   {
     return new Convertor(len, c) { private final int val$len;
   ...
   {
     return new Convertor(len) { private final int val$len;
       ...
       public Convertors.Date convert(Convertors.Date v) { return new Convertors.Date(v) { private int pos = this.val$v.len() - Convertors.4.this.val$len;
           ...
           public int len() {
             return Convertors.4.this.val$len;
           }
         };
       }
     };
   ...
   static Convertor pack()
   {
     return new Convertor() {
       public Convertors.Date convert(Convertors.Date v) {
         return new Convertors.Date(v) { private final Convertors.Date val$v;
   ...
   {
     return new Convertor(key) { private final Convertors.Date val$key;
   ...
   {
     return new Convertor(c2, c1) { private final Convertor val$c2;
     ...
     return new Convertor(key) { private final Convertors.Date val$key;
       ...
       public Convertors.Date convert(Convertors.Date v) { return new Convertors.Date(v) { private final Convertors.Date val$v;
           ...
           public int get(int i) { int n = this.val$v.get(i);
             if (n > 9)
               n = 0;
             return Convertors.10.this.val$key.get(n);
           }
   ...
   static abstract class Date
   {
     ...
     public Date reverse() {
       Date data = this;
       return new Date(data) { private final Convertors.Date val$data;
     ...
     private Date pad(int pos, int length, int c) {
       Date data = this;
       return new Date(pos, data, c, length) { private final int val$pos;
