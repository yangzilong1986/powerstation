   ...
   implements ICSExpParserTreeConstants, ICSExpParserConstants
 {
   ...
   private final boolean jj_2_1(int xla)
   {
     ...
     {
       int j = 1;
       return j; } finally { jj_save(0, xla); }
   }
   ...
   public ICSExpParser(InputStream stream, String encoding)
   {
     ...
     try
     {
       this.jj_input_stream = new JavaCharStream(stream, encoding, 1, 1); } catch (UnsupportedEncodingException e) { throw new RuntimeException(e); }
   ...
   private void jj_add_error_token(int kind, int pos)
   {
     ...
     if (pos == this.jj_endpos + 1) {
       ...
       if (pos == 0) return; this.jj_lasttokens[((this.jj_endpos = pos) - 1)] = kind;
     }
   }
