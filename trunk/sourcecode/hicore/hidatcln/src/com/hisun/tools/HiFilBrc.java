 ...
 public class HiFilBrc
 {
   ...
   public static void main(String[] args) throws Exception {
     ...
     for (int i = 2; i < args.length - 1; i += 2) {
       if (StringUtils.equalsIgnoreCase(args[i], "-s"))
         param._sysdate = args[(i + 1)];
       else if (StringUtils.equalsIgnoreCase(args[i], "-w"))
         param._workdate = args[(i + 1)];
   ...
   public void process(HiFilBrcParam param) throws Exception {
     ...
     if (StringUtils.equalsIgnoreCase("ALL", this._fileType))
       fileBpRecover.process(param);
     else
       fileBpRecover.process(this._fileType, param);
   }
