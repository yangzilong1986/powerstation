...
public class HiWorkLoadPool {
    ...
    public void init() {
        ...
        while (iter.hasNext()) ((HiWorkLoad) this.workLoadMap.get(iter.next())).init();
    }

    public static HiWorkLoadPool loadWorkLoadConfig(String file) throws HiException {
        ...
        HiWorkLoadPool localHiWorkLoadPool1 = null;
        try {
            HiWorkLoadPool workLoadPool = (HiWorkLoadPool) digester.parse(is);
            localHiWorkLoadPool1 = workLoadPool;
        } catch (Exception e) {
        } finally {
            try {
                is.close();
