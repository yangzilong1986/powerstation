...
public class HiAutoBootServerServlet extends HttpServlet {
    ...
    private void initConfig(ServletConfig config) throws Exception {
        String[] tmps;
        int i;
        ...
        if (StringUtils.isNotBlank(tmp)) {
            ...
            for (i = 0; i < tmps.length; ++i) {
                ...
                } else if (!(this.serverList.contains(server))) this.serverList.add(server);
            }
        }
    }
    ...
    private void startOneServer(HiServerNode server) throws Exception {
        ...
        if ("EJB".equalsIgnoreCase(HiICSProperty.getProperty("framework"))) doStartOneServerForEJB(server);
        else doStartOneServerForPOJO(server);
    }

    private void doStartOneServerForEJB(HiServerNode server) throws Exception {
