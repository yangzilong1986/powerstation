...
public class HiForm2Etf {
    ...
    public HiETF process(ServletRequest request, HiETF etf) throws Exception {
        ...
        while (en.hasMoreElements()) {
            ...
            if (values.length == 1) {
                ...
                if (!(this.ignoreCase)) group = etf.addNodeBase(groupName + "_1", "");
                else {
                ...
                if (!(this.ignoreCase)) etf.setChildValueBase(groupName + "_NUM", "1");
                else {
            ...
            for (int i = 0; i < values.length; ++i) {
                ...
                if (!(this.ignoreCase)) group = etf.getChildNodeBase(tmp);
                else {
                ...
                if (group == null) {
                    ...
                    if (!(this.ignoreCase)) group = etf.addNodeBase(tmp, "");
                    else {
            ...
            if (count != 0) {
                ...
                if (!(this.ignoreCase)) etf.setChildValueBase(groupName + "_NUM", String.valueOf(count));
                else {
    ...
    private void setValue(HiETF node, String name, String value) throws Exception {
        ...
        if (StringUtils.isNotBlank(value)) {
            ...
            if (!(this.ignoreCase)) node.setChildValueBase(name, value);
            else node.setChildValue(name, value);
        }
    }

    private void setValue01(HiETF node, String name, String value) throws Exception {
        ...
        for (int j = 0; j < nameValuePairs.length; ++j) {
            ...
            if (!(this.ignoreCase)) group = node.getChildNodeBase(tmp);
            else {
            ...
            if (group == null) {
                ...
                if (!(this.ignoreCase)) group = node.addNodeBase(tmp, "");
                else {
            ...
            if (nameValuesPair.length == 2) {
                ...
                if (!(this.ignoreCase)) group.setChildValueBase(nameValuesPair[0], tmp);
                else {
        ...
        if (count != 0) if (!(this.ignoreCase)) node.setChildValueBase(name + "_NUM", String.valueOf(count));
        else node.setChildValue(name + "_NUM", String.valueOf(count));
    }
}