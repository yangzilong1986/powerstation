...
$(document).ready(function(){
    autoCompleter("czmc","44",154);
});
</script>
</head>
<body onload="loadVols('${ETF.VOLT_GRADE}')">
<form action="112042.dow" method="post" name="ele"> 
<div id="body">
  <jsp:include page="archiveTabs.jsp" flush="true">
	...
	<div id="main">
      ...
      <table border="0" cellpadding="0" cellspacing="0" >
         <tr>
           <td width="70" class="label" align="center">电厂：</td>
           <td width="180">
		...
		<div class="content">
          ...
          <ul>
            <li class="target_on">
              <a href="#" onclick="setTarget(1); return false;">电厂列表</a>
            </li>
            <li class="clear"></li>
          </ul>
				</div>
	 ...
	 <script language=javascript>
       ...
       var first_able_Style="<a href=\"#\" id=\"queryNext\" onclick=\"pagemvFirst('112042.dow','WantedPerPage')\"><img style='margin-left:-5;margin-right:7;' src=\"<peis:contextPath/>/img/page_home.gif\" class=\"pointer\"/></a>\n";
       var forward_able_Style="<a href=\"#\" id=\"queryNext\" onclick=\"pagemvForward('112042.dow','WantedPerPage')\"><img style='margin-left:7;margin-right:7;' src=\"<peis:contextPath/>/img/page_prew.gif\" class=\"pointer\"/></a>\n";
       var next_able_Style="<a href=\"#\" id=\"queryNext\" onclick=\"pagemvNext('112042.dow','WantedPerPage')\"><img style='margin-left:7;margin-right:7;' src=\"<peis:contextPath/>/img/page_next.gif\" class=\"pointer\"/></a>\n";
       var end_able_Style="<a href=\"#\" id=\"queryNext\" onclick=\"pagemvEnd('112042.dow','WantedPerPage')\"><img style='margin-left:7;margin-right:7;' src=\"<peis:contextPath/>/img/page_end.gif\" class=\"pointer\"/></a>\n";
       ...
       function pagemvFirst(servce,param1){ 
        ...
        window.location.href=servce+"?"+param1+"="+strparam1+"&"+"&TIA_TYP=P&PAG_IDX=050000+&PAG_KEY="+tempPAG_KEY+"&VOLT_GRADE="+tempVOLT_GRADE+"&CZMC="+tempCZMC;
       }
       ...
       function getTargetPage(){
        ...
        var targetURL = "112042.dow?WANTEDPERPAGE="+$('#WantedPerPage').val()+"&PAG_KEY="+tempPAG_KEY + "&TIA_TYP=P&PAG_IDX=00"+page+"&VOLT_GRADE="+tempVOLT_GRADE+"&CZMC="+tempCZMC;
      ...
      function padLeft(str,lenght){
       ...
       function changePerRows(){
       		window.location.href="112042.dow?WANTEDPERPAGE="+$("#WantedPerPage").val()+"&PAG_IDX=aa+&PAG_KEY="+tempPAG_KEY+"&CZMC="+tempCZMC+"&VOLT_GRADE="+tempVOLT_GRADE;
       	}
...
function nextStep(){
   var tgNo=$(".selected td:eq(1)").attr("name");  //取到选中电厂
   var tgNo1=$(".selected td:eq(1)").attr("orgNo");  //取到选中电厂
   if(tgNo==null){
     alert("请选中一个电厂！");
   }else{
     window.location.href="<peis:contextPath/>/jsp/archive/addElecField.jsp?SUBS_ID="+tgNo+"&PARAM=1&ORG_NO="+tgNo1+"&CZMC="+tempCZMC+"&VOLT_GRADE="+tempVOLT_GRADE+"&WANTEDPERPAGE="+tempWantedPerPage+"&SUBS_TYPE=2";
   }
}
//删除变电站

function deleteTg(param,param1){
   if(confirm("是否删除电厂")==true){
     $.ajax({
      type:"POST",
      url:"112021.dow",
      dataType:"json",
      data:{SUBS_ID:param,SUBS_NO:param1,SUBS_TYPE:2},
...
function creatExcel(){
	...
	$.ajax({
      ...
      url:"112051.dow",
      dataType:"json",
      data:{CZMC:tempCZMC,VOLT_GRADE:tempVOLT_GRADE,FLG:"ELS"},
      success:function(data){
