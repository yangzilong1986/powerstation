...
function nextStep(archiveType){
  ...
  if(archiveType=="2"){//集中器及采集器
      ...
      $.ajax({
        ...
        cache: false,
        success: function(html){
          window.location.href=contextPath+"/jsp/archive/addLowCustTermFrame.jsp";
        }
      });
  }
  if(archiveType=="3"){//低压用户
      var url1=contextPath+"/archive/commAction.do?action=clearSessionByTg";
      $.ajax({
        url: url1,
        cache: false,
        success: function(html){
          window.location.href=contextPath+"/jsp/archive/addLowCustInfoSecond.jsp";
        }
      });
  }
}
