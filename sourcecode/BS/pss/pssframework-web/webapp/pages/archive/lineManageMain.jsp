...
//新增线路
function newLine(){
  var url=contextPath+"/jsp/archive/addLine.jsp";
  top.showDialogBox("新增线路",url, 575, 900);
}
//编辑线路 
function editLine(){
...
function delLine(){
  ...
  if(lineId==null){
    ...
    var url=contextPath+"/archive/lineInfoAcrion.do?action=delLine&lineId="+lineId;
    if(confirm("是否删除该条线路")==true){
         ...
         success: function(json){
           var msg=json['msg'];
            if(msg=="1"){
             alert("删除成功");
             top.getMainFrameObj().treeframe.location.href=contextPath+"/windowTreeAction.do?action=getTree&WINDOW=TREE&SMXBH=TREE_ARCHIVE_0002&objectType=10&queryFunction=query()&typicalType=1&limited=false&limitedtype=";//刷新树节点
             lineInfoForm.submit();
             }else if(msg=="5"){
...
<html:hidden property="action" value="queryLineList"/>
<html:hidden property="pageType" value="page"/>
<html:hidden property="sqlCode" value="AL_ARCHIVE_0052"/>
...
<div id="tool">
  ...
  <table border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td width="66" class="label">线路名称：</td>
          ...
          <logic:present name="PG_QUERY_RESULT">
            <logic:iterate id="dataInfo" name="PG_QUERY_RESULT">
              <tr onclick="selectSingleRow(this)" style="cursor:pointer;" id="<bean:write name="dataInfo" property="col1"/>">
                ...
                <td><c:if test="${dataInfo.col6!=null}"><fmt:formatNumber value="${dataInfo.col6*100}" pattern="#0.0000#"  minFractionDigits="0"/></c:if></td>
              </tr>
            </logic:iterate>
          </logic:present>
        </tbody>
