...
public class HiIBSComm implements IHandler, IServerInitListener {
    ...
    private void sendReceive(HiMessageContext ctx) throws HiException {
        ...
        if (this._cicsTxn != null) {
            if (msg.hasHeadItem(this._cicsTxn)) tpName = msg.getHeadItem(this._cicsTxn);
            else tpName = this._cicsTxn;
        } else {
        ...
        HiCPICComm cpic = new HiCPICComm();
        int ret = 0;
        try {
            ret = cpic.appcComm(this._cSideInfo, tpName, sendData.getBytes(), recvBuffer);
        } catch (CPICReturnCode e) {
            switch (cpic.getFlag()) {
                case -1:
                    throw new HiException("231207", "APPC", e);
                case -2:
                case -3:
            }
        }
        ...
        throw new HiException("231205", "APPC", e);
