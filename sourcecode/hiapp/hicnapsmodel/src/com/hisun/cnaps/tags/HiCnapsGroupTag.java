...
public class HiCnapsGroupTag extends HiCnapsSplitedTag {
    ...
    public void parse(String value) throws HiException {
        int endPos;
        int groupLength = value.length();
        if (groupLength % getLength() != 0) throw new HiException("241101", this.name);
    ...
    public String parse(HiETF etf) throws HiException {
        ...
        if (StringUtils.isBlank(subRepeat)) {
            while (true) {
                ...
                if ((subETF = etf.getChildNode(subEtfName)) == null) break label219;
                HiCnapsFixTag tag = new HiCnapsFixTag(getRootElement());
        ...
        while (++i <= subCnt) {
            ...
            if (subETF == null) throw new HiException("740005", subEtfName);
            HiCnapsFixTag tag = new HiCnapsFixTag(getRootElement());
            tag.setEtfname(subEtfName);
            addSubTag(tag);
        }
