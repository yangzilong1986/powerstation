...
import com.hisun.hiexpression.imp.*;
...
public class HiExpression implements ICSExpParserVisitor {
    private String exp;
    ...
    public String getValue(HiMessageContext ctx) throws HiException {
        try {
    ...
    public boolean isReturnTrue(HiMessageContext ctx) throws HiException {
        ...
        if (retValue instanceof Boolean) return ((Boolean) retValue).booleanValue();
        if (retValue instanceof String) {
            if (StringUtils.equals((String) retValue, "1")) return true;
    ...
    private Object getValueFromDS(HiMessageContext ctx, ASTIcsVarRef node) {
        ...
        if (o instanceof HiETF) return ((HiETF) o).getGrandChildValue(key);
        if (o instanceof HiMessage) return ((HiMessage) o).getHeadItem(key);
