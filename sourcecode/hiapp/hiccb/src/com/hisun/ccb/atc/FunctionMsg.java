...
public class FunctionMsg {
    public int addMsg(HiATLParam argsMap, HiMessageContext ctx) throws HiException {
        ...
        if ((msgInfo == null) || (msgInfo.equals(""))) msgInfo = (String) map.get("MSG_INF");
    ...
    public int outputErrorMsg(HiATLParam argsMap, HiMessageContext ctx) throws HiException {
        HiMessage mess = ctx.getCurrentMsg();
        HiETF etfRoot = (HiETF) mess.getBody();
    ...
    private void setResult(HiMessageContext ctx, String msgType, String msgCode) throws HiException {
        HiMessage mess = ctx.getCurrentMsg();
        HiETF etfRoot = (HiETF) mess.getBody();
        ...
        if (etfGWA == null) return;
        etfGWA.setChildValue("MSG_TYP", msgType);
        if (msgCode != null) etfGWA.setChildValue("MSG_CD", msgCode);
    }
}