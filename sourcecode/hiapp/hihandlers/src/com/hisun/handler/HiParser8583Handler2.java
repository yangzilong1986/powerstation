...
public class HiParser8583Handler2 implements IHandler, IServerInitListener {
    ...
    public void process(HiMessageContext ctx) throws HiException {
        ...
        if (this.log.isInfoEnabled()) this.log.info(sm.getString("HiParser8583Handler.parsered"));
    }

    public void convertHeaderToETF(ParseContext ctx) throws HiException {
        if (this.log.isInfoEnabled()) {
        ...
        if (this.log.isInfoEnabled()) this.log.info("Parser 8583 Header－End");
    }

    public void convertToETF(ParseContext ctx) throws HiException {
        makeEtfItem(ctx, this.items, 0);

        String bitMap = getBitMap(ctx);

        for (int i = 1; i < bitMap.length(); ++i)
            if (bitMap.charAt(i) == '1') makeEtfItem(ctx, this.items, i + 1);
    }

    private String getBitMap(ParseContext ctx) throws HiException {
        if (this.log.isInfoEnabled()) {
    ...
    private void checkCfg(Element cfgRoot, boolean isPutTlvList) throws HiException {
        ...
        if (item != null) {
            if (isPutTlvList) checkItemCfg(item, false, this.heads);
            else {
                checkItemCfg(item, false, null);
            }
        }

        if (isPutTlvList) checkItemCfg(cfgRoot, isPutTlvList, this.items);
        else {
    ...
    private void checkItemCfg(Element cfgRoot, boolean isPutTlvList, Map items) throws HiException {
    ...
    public void serverInit(ServerEvent arg0) throws HiException {
        ...
        if (this.log.isInfoEnabled()) this.log.info(sm.getString("HiParser8583Handler.init2", this.cfgFile));
    }

    private static byte[] readBytes(ByteArrayInputStream in, int length) throws HiException {
    ...
    public static class Parsers {
        public static HiParser8583Handler2.Parser seq(HiParser8583Handler2.Parser[] ps) {
            return new HiParser8583Handler2.Parser(ps) {
                private final HiParser8583Handler2.Parser[] val$ps;

                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    for (int i = 0; i < this.val$ps.length; ++i)
                        this.val$ps[i].parse(ctx);
                }
            };
        }

        private static int getHeadLen(ByteArrayInputStream in, int varLen, boolean bvar_bin) throws HiException {
            if (bvar_bin) {
                String len;
                int rsvLen = varLen % 2;
                ...
                if (rsvLen != 0) len = StringUtils.substring(bcd2Str(retBytes), 1);
                else len = bcd2Str(retBytes);
        ...
        public static HiParser8583Handler2.Parser varlen(int l, boolean bvar_bin) {
            return new HiParser8583Handler2.Parser(l, bvar_bin) {
                private final int val$l;
                private final boolean val$bvar_bin;

                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    ctx.len = HiParser8583Handler2.Parsers.access$100(ctx.in, this.val$l, this.val$bvar_bin);
                }
            };
        }

        public static HiParser8583Handler2.Parser lenParser(String field_id, Element item) throws HiException {
            ...
            if (length_type.equals("VAR2")) return varlen(2, bvar_bin);
        ...
        public static HiParser8583Handler2.Parser constlen(int len) {
            return new HiParser8583Handler2.Parser(len) {
                private final int val$len;

                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    ctx.len = this.val$len;
                }
            };
        }

        public static HiParser8583Handler2.Parser itemParser(Element item, boolean isPutTlvList) throws HiException {
            ...
            if (bTlv) n = tlvParser(field_id, item);
            else {
        ...
        private static void parserTlv(HiETF etfBody, byte[] dataBytes, String field_id, HiParser8583Handler2.Parser tlvParser, Logger log) throws HiException {
            if (dataBytes == null) {
                return;
            }
        ...
        public static HiParser8583Handler2.Parser tlvParser(String field_id, Element item) {
            ...
            return new HiParser8583Handler2.Parser(field_id, tlvParser) {
                private final String val$field_id;
                private final HiParser8583Handler2.Parser val$tlvParser;

                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    HiParser8583Handler2.Parsers.access$200(ctx.etf, ctx.retString.getBytes(), this.val$field_id, this.val$tlvParser, ctx.log);
                }
            };
        }
        ...
        public static HiParser8583Handler2.Parser saveParser(String field_id, Element item) {
            ...
            return new HiParser8583Handler2.Parser(itemName, field_id) {
                private final String val$itemName;
                private final String val$field_id;

                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    ctx.etf.setGrandChildNode(this.val$itemName, ctx.retString);
        ...
        public static HiParser8583Handler2.Parser dataParser(String field_id, Element item) throws HiException {
        ...
        private static String readBcd(ByteArrayInputStream in, int length, int radix, boolean balign_left, boolean bSpec, String pro_dll1, String pro_func1, Logger log) throws HiException {
        ...
        private static byte[] specProc(Logger log, String pro_dll1, String pro_func1, byte[] values) throws HiException {
        ...
        public static HiParser8583Handler2.Parser bcd(boolean balign_left, boolean bSpec, String pro_dll1, String pro_func1) {
            return new HiParser8583Handler2.Parser(balign_left, bSpec, pro_dll1, pro_func1) {
                private final boolean val$balign_left;
                private final boolean val$bSpec;
                private final String val$pro_dll1;
                private final String val$pro_func1;

                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    ctx.retString = HiParser8583Handler2.Parsers.access$300(ctx.in, ctx.len, 2, this.val$balign_left, this.val$bSpec, this.val$pro_dll1, this.val$pro_func1, ctx.log);
                }
            };
        }
        ...
        public static HiParser8583Handler2.Parser ascii(boolean bconvert, boolean bSpec, String pro_dll1, String pro_func1) {
            return new HiParser8583Handler2.Parser(bSpec, pro_dll1, pro_func1, bconvert) {
                ...
                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    ...
                    if (this.val$bconvert) data_value = HiParser8583Handler2.Parsers.access$500(retBytes);
                    else {
        ...
        public static HiParser8583Handler2.Parser tlv(Element item) {
            ...
            return new HiParser8583Handler2.Parser(tags, field_id) {
                private final Map val$tags;
                private final String val$field_id;

                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    parserTlvItem(ctx, ctx.etf, ctx.in);
                }
                ...
                private int getTlvlen(ByteArrayInputStream in) throws HiException {
                    int length = 0;
                    byte[] bytes = new byte[1];
        ...
        public static HiParser8583Handler2.Parser tlvValue(String tag, String data_type, String etf_name) {
            ...
            return new HiParser8583Handler2.Parser(isasci, isbcd, tag, etf_name) {
                private final boolean val$isasci;
                private final boolean val$isbcd;
                private final String val$tag;
                private final String val$etf_name;

                public void parse(HiParser8583Handler2.ParseContext ctx) throws HiException {
                    byte[] bytes = ctx.retBytes;
                    int length = ctx.len;
                    String data_value = "";
    ...
    public static abstract interface Parser {
        public abstract void parse(HiParser8583Handler2.ParseContext paramParseContext) throws HiException;
    }
