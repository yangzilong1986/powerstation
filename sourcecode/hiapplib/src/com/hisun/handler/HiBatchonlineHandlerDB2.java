   ...
   implements IHandler, IServerInitListener, IServerStartListener, IServerStopListener, IServerPauseListener, IServerDestroyListener, IServerResumeListener
 {
   ...
   public void process(HiMessageContext ctx) throws HiException {
     ...
     if (this._serverLog.isInfoEnabled())
       this._serverLog.info("批量发送通道处理开始");
     while (true)
     {
       if (this._status == 2) {
       ...
       if (this._serverType.equals("B"))
         checkServerTypeForB(dbUtil, actDat);
       else {
         checkServerTypeForA(dbUtil, actDat);
       }
 
       try
       {
         record = dbUtil.readRecord("SELECT * FROM PUBBATINF WHERE CMT_FLG = '%s'  ORDER BY Lst_Tm FETCH FIRST ROW ONLY", this._serverType);
       }
       catch (HiException e)
       {
         while (true) {
       ...
       if (!(StringUtils.equals(actDat, batchInfo.acc_dt)))
       {
         ...
         try
         {
           dbUtil.execUpdate("UPDATE pubbatinf SET CMT_FLG = 'S' WHERE  DSK_NO = '%s'", batchInfo.dsk_no);
     ...
     throws HiException
   {
     int ret = 0;
     try {
       ret = dbUtil.execUpdate("UPDATE pubbatinf SET CMT_FLG = 'B' WHERE DSK_NO IN (SELECT  min(DSK_NO) FROM pubbatinf WHERE ACC_DT = '%s' AND CMT_FLG = '0' AND TXN_MOD = '1' AND CHK_FLG = '1' AND CAST(ORN_CNT AS INT) >= %s)", actDat, String.valueOf(this._level));
     ...
     throws HiException
   {
     int ret = 0;
     try {
       ret = dbUtil.execUpdate("UPDATE pubbatinf SET CMT_FLG = 'A' WHERE DSK_NO IN (SELECT  min(DSK_NO) FROM pubbatinf WHERE ACC_DT = '%s' AND CMT_FLG = '0' AND TXN_MOD = '1'  AND CHK_FLG = '1' AND CAST(ORN_CNT AS INT) < %s)", actDat, String.valueOf(this._level));
     ...
     throws HiException
   {
     ...
     try
     {
       ...
       while (this._status != 1)
       {
         ...
         if (msg.getHeadItem("STF") != null)
           newMsg.setHeadItem("STF", msg.getHeadItem("STF"));
         else {
           newMsg.setHeadItem("STF", "0");
         }
         newMsg.setHeadItem("STF", "1");
     ...
     if (StringUtils.equals(batchInfo.sum_flg, "Y"))
     {
       ...
       if ((record != null) && (!(record.isEmpty()))) {
         ...
         if (StringUtils.isBlank(sucCnt))
           batchInfo.suc_cnt = "0";
         else {
           batchInfo.suc_cnt = sucCnt;
         }
         if (StringUtils.isBlank(sucAmt))
           batchInfo.suc_amt = "0";
         else {
     ...
     throws HiException
   {
     ...
     String sqlCmd = "UPDATE pubbatinf SET Snd_Cnt='%s', Lst_Tm='%s' WHERE Dsk_No='%s'";
     try {
       dbUtil.execUpdate(sqlCmd, String.valueOf(snd_cnt), String.valueOf(System.currentTimeMillis() / 1000L), batchInfo.dsk_no);
   ...
   private void sleep(int second, Logger log) {
     try {
       Thread.currentThread(); Thread.sleep(second * 1000);
   ...
   public void setNumOfLevel(String NumOfLevel) throws HiException {
     ...
     if ((this._level > 500) || (this._level < 200))
       throw new HiException("231416", NumOfLevel);
   }
 
   public void setNumOfPthread(String NumOfPthread) throws HiException
   {
     this._numOfPthread = Integer.parseInt(NumOfPthread);
     if ((this._numOfPthread > 20) || (this._numOfPthread < 1))
       throw new HiException("231415", NumOfPthread);
   }
     ...
     throws HiException
   {
     ...
     this._pool = HiThreadPool.createThreadPool(DEFAULT_MIN_THREADS, this._numOfPthread);
     ...
     implements Runnable
   {
     ...
     public void run() {
       ...
       ctx.pushParent(HiBatchonlineHandlerDB2.this._serverContext);
       ctx.setCurrentMsg(this.msg);
       HiMessageContext.setCurrentMessageContext(ctx);
       try {
         process(ctx);
       } catch (Throwable t) {
         HiBatchonlineHandlerDB2.this._serverLog.error(t, t);
       } finally {
     ...
     {
       String logNo = null; String updFlg = null; String stmt1 = null; String batTbl = null;
       String aHTxnSt = null; String hLogNo = null; String hRspCd = null; String tlrId = null;
       HiDataBaseUtil dbUtil = ctx.getDataBaseUtil();
       ...
       for (i = 0; i < 1; ++i) {
         ...
         {
           synchronized (HiBatchonlineHandlerDB2.this._lock) {
             label585: HiBatchonlineHandlerDB2.access$408(HiBatchonlineHandlerDB2.this);
           }
         }
         ...
         if (log.isInfoEnabled()) {
           log.info("batchOnline-stmt1=[" + stmt1 + "]");
         }
         try
         {
     ...
     implements Runnable
   {
     public void run()
     {
       ...
       try {
         ctx.pushParent(HiBatchonlineHandlerDB2.this._serverContext);
         HiMessage msg = new HiMessage(HiBatchonlineHandlerDB2.this._serverName, "PLTIN0");
         ctx.setCurrentMsg(msg);
         HiMessageContext.setCurrentContext(ctx);
         HiBatchonlineHandlerDB2.this.process(ctx);
       } catch (Throwable t) {
         HiBatchonlineHandlerDB2.this._serverLog.error(t, t);
       } finally {
         if (ctx.getDataBaseUtil() != null)
